    ...
    private void handleClient(Socket clientSocket)
    {
        Thread cilentThread = new Thread(new Runnable()
        {
            ...
            public void run()
            {
                ...
                log(clientId + " connected frodddddddddsssssssddm " + clientAddress);
                // create Connection object to the new client
                Connection conToClient = new Connection(clientSocket, clientId);
                log(clientId + " connected frodddddddddddm " + clientAddress+"hhhhhhh"+clientsList.size());
                // add the new Connection object to the clients list
                clientsList.add(conToClient);
                log("clientsList.size()"+clientsList.size());
                if(clientsList.size()==2)
                {
                    System.out.println("in");
                    Game game = new Game(new PlayerNet(clientsList.get(0)), new PlayerNet(clientsList.get(1)));
                        clientsList.get(0).writeMessage(Make.open);
                           clientsList.get(1).writeMessage(Make.open);
                }
                else
                {
                   log("sand wait to himhhhhhhhhhhhhhhhhhhhhh");
                    conToClient.writeMessage(Make.wait);
                }
                
                // send to the new client his ID 
                ...
                while(true)
                {
                    ...
                    Message msg = conToClient.readMessage();
