...
public class Model {
    ...
    private static int Blackconter ;
    private static int whiteconter ;
    ...
    boolean hasReleases(Location btnloc, Turn turn) {
        if (turn == Turn.black) {
            if (hasEnd(btnloc) && logicMat[btnloc.getRow()][btnloc.getCol()].getValue() == BLACKing && !blackCaptives.isEmpty()) {
                return true;
            }
        } else if (turn == Turn.white) {
            if (hasEnd(btnloc) && logicMat[btnloc.getRow()][btnloc.getCol()].getValue() == WHITEKING && !whiteCaptives.isEmpty()) 
            {
                return true;
            }
