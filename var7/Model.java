...
//    }
    ...
    private ArrayList<Move> GetAllCompMoves(Turn player) {
        ...
        ArrayList<Location> now = getSoldiersNoFinishers(player);
        ArrayList<Location> ToGo;
        for (int i = 0; i < now.size(); i++) {
            ToGo = GetPosiblePlace(now.get(i), player);
            for (int j = 0; j < ToGo.size(); j++) {

                Move move = new Move(player, player, ToGo.get(i), now.get(i));
                list.add(move);
            }
...
//    void doMove(Turn turn, Turn NOturn, Location btnloc, Location clickedLocation) 
//    {
//        if (hasEnemy(turn, btnloc)) {
//
//            capture(clickedLocation, btnloc, turn, NOturn);
//
//            go(clickedLocation, btnloc);
//
//        } else {
//            if (itEmpty(btnloc)) {
//
