...
public class Controller {
    ...
    public static Clicking getUniversalclick() {
        return Universalclick;
    }
  ...
  static  void   shit(){
    ...
    static void pcMove() {
        ...
        Move move = model.miniMax(turn, Double.MIN_VALUE, Double.MAX_VALUE, 10, 10, 5000);
        view.lockUndo(true);
        if(Universalclick ==Clicking.secondClick)
        view.coloringList(model.GetPosiblePlace(clickedLocation, turn), view.Normal);
          if(Universalclick == Clicking.reviveClick)
              System.out.println("var8.Controller.pcMove()");
        
     
        model.applyMove(move);
        view.updateGUI();
        if (model.hasReleases(move.getToGo().getLocYesSvohim(), turn)) 
        {
            view.applyReleases(model.GetTheShvoim(turn), turn, move.getNow().getLocYesSvohim());
            Universalclick = Clicking.reviveClick;
            return;
        }
        if (model.hasEnd(move.getToGo().getLocYesSvohim())) {
            model.Raisefinishing(turn);
        }
        ...
        view.setClick(Clicking.firstClick, move.getNow().getLocYesSvohim());
        Universalclick = Clicking.firstClick;
        ReplaceTurn(NOturn, turn);
